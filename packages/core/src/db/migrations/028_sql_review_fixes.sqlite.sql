-- ---------------------------------------------------------------------------
-- 028: SQL Review Fixes — performance indexes (SQLite version)
-- ---------------------------------------------------------------------------
...
-- line_items: critical for every balance calculation
...
-- revenue_schedules: lookup by subscription + status
...
CREATE INDEX IF NOT EXISTS idx_notifications_user_ledger
  ON notifications (ledger_id, user_id, created_at);
...
-- ============================================================================
-- 2. Audit entries immutability triggers (SQLite)
-- ============================================================================

CREATE TRIGGER IF NOT EXISTS trg_audit_no_update
  BEFORE UPDATE ON audit_entries
  BEGIN
    SELECT RAISE(ABORT, 'audit_entries is append-only: UPDATE operations are forbidden');
  END;

CREATE TRIGGER IF NOT EXISTS trg_audit_no_delete
  BEFORE DELETE ON audit_entries
  BEGIN
    SELECT RAISE(ABORT, 'audit_entries is append-only: DELETE operations are forbidden');
  END;
